...
class DHVT_Block(TimestepBlock):

    def __init__(self, dim, num_heads, time_embed_dim, num_tokens = 197, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., qk_scale=None, 
        ...
        self.embed_layers = nn.Sequential(
                act_layer(),
                nn.Linear(time_embed_dim, num_tokens)
                )        
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x, time_embed, skip=None):
        B, N, C = x.shape
        if self.skip_linear is not None:
            x = self.skip_linear(torch.cat([x, skip], dim=-1))
            
        emb_out = self.embed_layers(time_embed).type(x.dtype)
...
class DHUNet(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=16, in_chans=3, mlp_time_embed = False, num_classes=100, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', apply_dhvt=True, conv=True, skip=True):
        ...
        if self.num_classes is not None:
            ...
            self.extras = 1
        emb_tokens = (img_size//patch_size)**2
        time_embed_dim = emb_tokens * 4
        self.time_embedding = nn.Sequential(
            PositionalEmbedding(emb_tokens, 1),
            nn.Linear(emb_tokens, time_embed_dim),
            act_layer(),
        ...
        if self.apply_dhvt:
            self.in_blocks = nn.ModuleList([TimestepEmbedSequential(
                DHVT_Block(
                    dim=embed_dim, num_heads=num_heads, time_embed_dim = time_embed_dim, num_tokens = emb_tokens+1, mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                    act_layer=act_layer))
                for i in range(depth // 2)])
            self.mid_block = TimestepEmbedSequential(DHVT_Block(
                    dim=embed_dim, num_heads=num_heads, time_embed_dim = time_embed_dim, num_tokens = emb_tokens+1, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, act_layer=act_layer))
            
            self.out_blocks = nn.ModuleList([TimestepEmbedSequential(
                DHVT_Block(
                    dim=embed_dim, num_heads=num_heads, time_embed_dim = time_embed_dim, num_tokens = emb_tokens+1, mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, act_layer=act_layer, 
                    skip=skip))
            ...
            self.out_blocks = nn.ModuleList([
                DHVT_Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, act_layer=act_layer, skip=skip)
                for i in range(depth // 2)])
    ...
    def forward_features(self, x, timesteps, y = None):
        B, c, h, w = x.shape
...
#         print("1>>>", x.shape)
        x = self.forward_features(x, timesteps = timesteps, y=y)
#         print("2>>>", x.shape)
        x = self.norm(x)
        x = self.decoder_pred(x)
#         print("3>>>", x.shape)
        assert x.size(1) == self.extras + self.L
        x = x[:, self.extras:, :]
