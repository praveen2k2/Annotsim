...
if __name__ == '__main__':
    ...
    # build model
#     model = UViT(img_size = args.image_size[0], patch_size=16, in_chans=3, embed_dim = 768,
...
#                  use_checkpoint=False, conv=True, skip=True)
    ...
    if os.path.isfile(args.pretrained_weights):
        ...
        if "ema" in state_dict:
            msg = model.load_state_dict(state_dict["ema"])
        else:
...
#     lab = None
    lab = torch.tensor([1], device='cuda:0')
    ...
    samples = diffusion.forward_backward(
        ...
        t_distance=timestep, denoise_fn = 'gauss'
    )
    outputs = torch.cat(
        (
            img.to(device), 
